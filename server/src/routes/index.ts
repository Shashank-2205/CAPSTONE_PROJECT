...
const buildSummary = (role: string) => {
  ...
  const activeRescues = emergencyStore.filter((item) =>
    ['Assigned', 'In Progress', 'Verified'].includes(item.status),
  ).length;
  const resourceShortages = resourceStore.filter((item) => item.quantity < 20).length;

  const roleDefaults: Record<string, { availableVolunteers: number; resourceShortages: number }> = {
